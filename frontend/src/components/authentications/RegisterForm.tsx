...
const RegisterForm = ({ regFn }: Props) => {
  ...
  const genders: string[] = ["Not Known", "Male", "Female", "Indeterminate"];
  const {
    // screenSmall,
    // screenMedium,
    screenLarge,
        ...
        <form onSubmit={handleSubmit(onSubmitHandler)}>
          ...
          <div className="w-72 pb-4 flex flex-row justify-around items-stretch gap-3">
            <div>
              ...
              >
                {Array.from(
                  ...
                  return <option value={index + 1920}>{y + 1919}</option>;
                })}
              </select>
            </div>
            <div>
              ...
              >
                {Array.from({ length: 12 }, (_, i) => i + 1).map((y, index) => {
                  return (
                    <option value={index}>
            ...
            <div>
              ...
              >
                {Array.from({ length: 31 }, (_, i) => i + 1).map((y, index) => {
                  return <option value={index}>{y}</option>;
                })}
              </select>
            </div>
          </div>
          ...
          <div className="w-72 pb-4 flex flex-row justify-around items-stretch gap-3">
            {genders.map((g) => {
              return (
                <div>
                  <input type="radio" name="gender" value={g} id={g}>
                    {t(g)}
                  </input>
                </div>
              );
